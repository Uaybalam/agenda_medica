...
class DynamicRoutes
{
	 ...
	 *		#routes_path 			: file output
	 * 		#controller_path 		: path controllers
	 * 		#format 				: data type json|serialize
	...
	public function get_current_routes()
	{
		...
		{	
			if($this->format==='json')
			{
				$currentRoutes =  @json_decode( 
					file_get_contents($this->routes_path),
				true);
			}
			else if($this->format==='serialize')
			{
				$currentRoutes = @unserialize(
					file_get_contents($this->routes_path)
				);
			}
			else
			{	
				$this->_error("Format type not found [".$this->format."] use: [json or serialize]");
			}

...
class Pagination{
	...
	public function __construct( Array $config , $columns = '*' )
	{
		...
		if(isset($config['filters']) && is_array($config['filters']) && count($config['filters'])>0)
		{
			$config['filters'] = $config['filters'];
		}
		else if( isset($_GET['filters']) && is_array($_GET['filters']) && count($_GET['filters'])>0 )
		{
			$config['filters'] = (Array)$_GET['filters'];
		}
		else
		{
			$config['filters'] = [];
		...
		if(isset($config['sort'])  || !is_array($config['sort']) || count($config['sort']) === 0)
		{
	...
	public function retrieve( \Closure $CallBack = null ) 
	{
		...
		if($CallBack && is_callable($CallBack))
		{
			...
			$temporalQB = (get_class($responseQB) === 'CI_DB_pdo_mysql_driver') ? $responseQB : $countQueryBuilder;
		...
		{	
			...
			if( get_class($responseQB) === 'CI_DB_pdo_mysql_driver')
			{
	...
	public function setSelectCount(\Closure $CallBack = null)
	{
		...
		$countQueryBuilder          = $this->CI->db;
		$countQueryBuilder->from( $this->getConfig('table') );
		$responseQB                 = $CallBack( $countQueryBuilder );
	...
	protected function searchData( $queryBuilder  )
	{
		...
		$limit = $itemsPerPage;//($itemsPerPage==0 || $itemsPerPage>1000) ? 15 : $itemsPerPage;
        ...
        {
        	$queryBuilder->order_by( $this->getConfig('table').'.id DESC');
        }

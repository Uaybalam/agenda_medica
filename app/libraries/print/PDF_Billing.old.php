...
class PDF_Billing extends FPDF_Extends{
	...
	{	
		...
		$this->_facility->address = $this->administration->getValue('billing_facility_street_addr');
    ...
    {	
		...
		$this->_pos( 103.5, 8.5 , 'C')->_cell($bill->patient->gender ==1 ? 'X'  : '');
		$this->_pos( 116.5, 8.5 )->_cell($bill->patient->gender ==2 ? 'X'  : '');
		...
		//Line_10
		$this->_pos( 0, 76.5 )->_cell( strtoupper($bill->insured_other_insurance_plan_name) , 72.5);
		...
		$this->_pos( 142, 76.5 )->_cell( $bill->other_benefit_plan ==='No' ? 'X' : '');
		//Line_11
		$this->_pos( 89 , 92 )->_cell( date('m/d/Y', strtotime($bill->done_at)), 35);
		//Line_12
		$this->_pos( 2 , 102 , 'C')->_cell( date('m', strtotime($bill->done_at) ), 7);
		$this->_pos( 9 , 102 , 'C')->_cell( date('d', strtotime($bill->done_at) ), 8);
		$this->_pos( 17 , 102 ,'C')->_cell(date('Y', strtotime($bill->done_at) ), 10);
		...
		$this->_pos( 0 , 118.5 )->_cell( $bill->aditional_claim , 123);
		...
		//Fotter_Line_3
		$this->_pos( 124, 216)->_cell( strtoupper($this->_facility->name.', '.$this->_facility->address) , 76.5 );
		//Fotter_Line_4
		$this->_pos( 32 , 220 )->_cell( date('m/d/Y', strtotime($bill->done_at)), 24 );
		$this->_pos( 124, 220)->_cell( strtoupper( $this->_facility->city.', '.$this->_facility->state.', '.$this->_facility->city) , 76.5 );
		//Fotter_Line_5
		$this->_pos( 130, 224 )->_cell( $this->_provider->group_npi, 29 );
    }
	...
	private function _services( $top, $service , $m , $d , $y)
	{
		...
		$this->_pos( 60.5 , $top )->_cell( $service->procedure_cpt_hcpcs, 18.5);
		$this->_pos( 79 , $top )->_cell( $service->modifier_a, 9);
